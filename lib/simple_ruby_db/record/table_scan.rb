    ...
    class TableScan
      ...
      def value(field_name)
        if layout.schema.field_type(field_name) == 'integer'
          return SimpleRubyDb::Query::Constant.new(get_int(field_name))
        end

        SimpleRubyDb::Query::Constant.new(get_string(field_name))
      end
      ...
      def move_to_new_page
        ...
        @record_page = SimpleRubyDb::Record::RecordPage.new(buffer_pool_manager, new_page.page_id, layout)
        record_page.format
      end
      ...
      def move_to_page(page_id)
        ...
        @record_page = SimpleRubyDb::Record::RecordPage.new(buffer_pool_manager, page_id, layout)
      end
